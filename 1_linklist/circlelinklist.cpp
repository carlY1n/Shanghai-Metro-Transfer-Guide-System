...
void CircleLinkList::insertAtIndex(int index, int val)//在索引index位置插入值为val的元素
{
...
void CircleLinkList::deleteAtIndex(int index)//删除索引index处的节点
{
...
void CircleLinkList::modifyAtIndex(int index, int val)//将index处节点的元素值修改为val
{
    if (index < 0 || index >= length)
    {
