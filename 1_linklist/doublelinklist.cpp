...
void DoubleLinkList::insertAtIndex(int index, int val)//在索引index位置处插入值为val的元素
{
...
void DoubleLinkList::deleteAtIndex(int index)//删除索引index处的节点
{
...
void DoubleLinkList::modifyAtIndex(int index, int val)//将index处节点的元素值修改为val
{
    if (index < 0 || index >= length)
    {
